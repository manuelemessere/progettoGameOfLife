...
class GameOfLife:
    ...
    def create_grid(self):
        ...
        return grid
    '''
    #Forse, però, è più saggio usare quella di prima sebbene ridondante perché il l*l*sigma  è dato in consegna e in questa nuova non usa pdr
...
class GameOfLifeGUI:
    ...
    def setup_Gui(self): # Ho aggiunto una nuova funzione che contenga i frame e label su suggerimento perché più ordinato
       ...
       #Ho tolto il pulsante di salvataggio perché diventato inutile dato che viene richiesto automaticamente in draw_turtle
    ...
    def save_simulation(self):
        ...
        try:
            with open(filename, 'x') as file: #la x sta per "modalità scrittura esclusiva" e viene gestitoda FileExistsError
            ...
            while True:
                risposta = messagebox.askyesno(message='Si vuole sovrascrivere il file?')
                if risposta:
                    ...
                    messagebox.showinfo('Salvataggio', 'Simulazione sovrascritta correttamente.')
                    break
                
                else:
                    nuovo_filename = tk.filedialog.asksaveasfilename(title='Salva Simulazione', defaultextension='.txt')
                    if not nuovo_filename:
                # L'utente ha annullato il salvataggio
...
""" 
MODO ALTERNATIVO CHE INVECE DI SOLLEVARE L'ECCEZIONE FILEEXISTSERROR, USA OS.PATH.EXISTS
...
def save_simulation(self):
    ...
    if os.path.exists(filename):
        risposta = messagebox.askyesno(message='Il nome è già stato utilizzato. Si desidera sovrascrivere?')
        if not risposta:
            # Chiedi un nuovo nome del file
            nuovo_filename = tk.filedialog.asksaveasfilename(
                title='Salva Simulazione', defaultextension='.txt')
            if not nuovo_filename:
                # L'utente ha annullato il salvataggio
                return
